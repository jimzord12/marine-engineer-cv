#let d = json("../content/company-example.json")
#let navy = rgb("102f3a")
#let teal = rgb("236a70")
...
// Body spacing is retained from revision 06; this revision changes only the hero.
...
#let company(c) = block(breakable: false, above: 0pt, below: 8mm)[
  ...
  #grid(columns: (31mm, 1fr), column-gutter: space.md,
    stack(spacing: 2mm,
      ...
      block[
        ...
        #text(font: "Barlow", size: 19pt, fill: teal)[#if months >= 12 and calc.rem(months, 12) > 0 {
    ...
    block(stroke: (left: 2pt + teal), inset: (left: space.md, top: 0.5mm, bottom: 1mm))[
      ...
      #for group in c.groups [
        ...
        #v(3mm)
        #grid(columns: (1fr, 31mm, 28mm), column-gutter: 2mm, row-gutter: 3.5mm, align: (left, left, right),
          ..group.ships.map(ship => (
            text(size: 10.5pt, weight: "semibold")[#ship.name],
            text(size: 9pt, fill: grey)[#ship.rank],
            text(size: 9pt, fill: teal)[#duration(ship.months)]
          )).flatten())
      ]
    ])
]
...
#for c in d.companies { company(c) }
#v(1fr)
#block(fill: navy, width: 100%, inset: (x: space.lg, y: space.md))[
 #set text(fill: white)
 #grid(columns: (1.4fr, 1fr, 1fr), column-gutter: 5mm,
   stack(spacing: space.xs, text(font: "Barlow", size: 25pt)[#duration(total-months)], label("Total experience", color: brass)),
   stack(spacing: space.xs, text(font: "Barlow", size: 25pt)[#ships.map(s => s.id).dedup().len()], label("Vessels", color: brass)),
   stack(spacing: space.xs, text(font: "Barlow", size: 25pt)[#d.companies.len()], label("Companies", color: brass)))
]
#pagebreak()
#grid(columns: (1fr, auto), align: horizon,
  stack(spacing: 1mm, text(font: "Barlow", size: 23pt)[#d.name], label(d.rank, color: teal)),
  text(size: 9pt, fill: grey)[CREDENTIALS / EDUCATION])
#v(space.sm)
...
#heading("03", "Education & languages")
