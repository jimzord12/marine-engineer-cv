...
#let grey = rgb("546870")
#let mist = rgb("eef4f3")
#set document(title: "Alex Morgan | Second Engineer | Flagship prototype", author: "Marine CV Studio")
...
#let voyage(trip, i, compact: false) = block(breakable: false, below: 4mm)[
  #grid(columns: (31mm, 1fr), column-gutter: 5mm,
    ...
    block(stroke: (left: 2pt + teal), inset: (left: 5mm, top: 1mm, bottom: 2mm))[
      ...
      #v(4pt)
      #text(size: 10pt)[#trip.description]
    ])
]
...
#grid(columns: (1fr, 1fr), column-gutter: 8mm,
 ..d.skills.map(s => stack(spacing: 4pt, text(weight: "bold", fill: teal)[#s.at(0)], text(size: 10pt)[#s.at(1)])))
#v(5mm)
